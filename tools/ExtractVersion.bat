...
set VERSION_NUM=
for /F "tokens=1,2,3 delims= " %%a in ('type "%CUR_DIR%..\version.h"') do (
...
for /F "tokens=1,2,3 delims= " %%a in ('type "%CUR_DIR%..\version.h"') do (
if "%%a"=="#define" if "%%b"=="BETA_NUM" set BETA_STATE=-beta%%~c
)
:SkipBeta
...
:end
echo Extraxted version: %VERSION_NUM%%BETA_STATE%%REVISION_NUM%
exit /b 0